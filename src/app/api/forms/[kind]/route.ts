...
export async function POST(request: NextRequest, { params }: { params: Promise<{ kind: string }> }) {
  ...
  if (!process.env.BREVO_API_KEY || !process.env.ORGANIZATION_EMAIL) return NextResponse.json({ ok: true });
  const payload = { sender: { email: process.env.BREVO_SENDER_EMAIL || process.env.ORGANIZATION_EMAIL, name: process.env.BREVO_SENDER_NAME || "Konform’up" }, to: [{ email: process.env.ORGANIZATION_EMAIL }], subject: `[Site] ${kind === "quote" ? "Demande de devis" : "Message de contact"}`, textContent: ["Nouvelle demande reçue depuis le site public.", `Nom : ${clean(input.firstName, 160)} ${clean(input.lastName, 160)}`, `E-mail : ${clean(input.email, 160)}`, `Message : ${clean(input.message)}`].join("\n") };
